...
Renderer.prototype.draw = function() {
  for (var i = 0, max = world.bodies.length; i < max; i++) {
    ...
    for (var j = 0; j < maxFixtures; j++) {
      ...
      fixture.shape.draw(transform, drawLineColors && body.userData ? body.userData.color : [0,0,0], body.userData);
    }
  }
...
b2PolygonShape.prototype.draw = function(transform, color, userData) {
  ...
  renderer.transformVerticesAndInsert((!debugPolygons && userData && userData.vertices) ? userData.vertices : this.vertices, transform, color[0],color[1],color[2]);
