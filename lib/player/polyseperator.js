function b2Seperator() {
  var threshold = 0.0000005;
   ...
   this.Seperate = function Separate(body,fixtureDef,verticesVec,scale) {
     ...
     //Calculate total area of figsVec
     //Remove any figsVec smaller than x fraction of total area
   ...
   function isOnSegment(px,py,x1,y1,x2,y2) {
     var b1=((((x1+threshold)>=px)&&px>=x2-threshold)||(((x1-threshold)<=px)&&px<=x2+threshold));
     var b2=((((y1+threshold)>=py)&&py>=y2-threshold)||(((y1-threshold)<=py)&&py<=y2+threshold));
